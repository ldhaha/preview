...
1.首先进行url解析，例如http://www.baidu.com/123，将url分解出协议(https://)，主机名(www.baidu.com)，端口号，路径/123，如果输入的不是url，则会用默认引擎进行搜索

2.DNS解析，将域名转为机器可读的ip地址，首先检查浏览器缓存看看是否存在该域名的IP，其次是操作系统和本地hots文件，再者查看路由器缓存，如果都没有，则去根域名服务器->顶级域名服务器->权威域名服务器查找

3.TCP连接（三次握手）

（1）浏览器向服务器发送一个syn包（序列号为x）,表示请求建立连接，进入同步已发送状态

（2）服务器收后，回复一个syn-ack包，序列号为y，ack为x+1

（3）浏览器再回复一个ack包（确认号为y+1）

此时建立连接成功，如果是https的话，三次握手之后还要进行TLS握手

4.连接建立后，浏览器向服务器发送一个http请求报文

5.服务器处理并返回响应

6.关闭tcp连接，默认情况http/1.1会使用Keep-Alive机制，连接不会立即关闭，如果不需要会进行四次挥手来关闭连接

7.浏览器处理相应内容，构建DOM树，遇到CSS会去构建CSSDOM树，当遇到script标签时，则会暂停解析

8。根据DOM树和CSSDOM树构建渲染树，进行布局（重排），以及绘制
...
前者在所有同步代码执行结束。会在defer之后执行，而后者会在包括async之后执行
...
#### Flex:1,flex:auto,flex-grow,flex-shrink,flex-basis

flex: [flex-grow] [flex-shrink] [flex-basis]

flex:1 => flex:1 1 0.  多个元素会等分

flex:auto => flex:1 1 auto 先根据项目内容本身的大小，再去瓜分
...
前者是当元素的尺寸，布局，隐藏等发生改变时，需要重新计算每个元素在视口的位置
